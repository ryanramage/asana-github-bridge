...
function parseConfig(parsed) {
  ...
  // Behaviour for events arriving with no registered handler. Defaults to
  // true so that orgs that webhook everything ("Send me everything") don't
  // see GitHub deliveries marked as failed for events the bridge doesn't
  // care about (star.created, watch.started, etc.). Set to false to make
  // unhandled events visible as 404s in the GitHub webhook UI.
  const acceptUnknownEvents = parsed.acceptUnknownEvents !== false;
